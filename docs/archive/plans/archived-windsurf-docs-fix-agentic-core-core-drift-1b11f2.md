...
original_path: 'C:\\Git\\windsurf-plans-recovered\\docs_reports_plans\\fix-agentic-core-core-drift-1b11f2.md'
original_relative_path: 'fix-agentic-core-core-drift-1b11f2.md'
source_sha256: 4fbd88e2153b8fdaf50ec37b14fd6a1513349774908617a646464a0749ebe2a1
...
**NOTE**: This plan file must be relocated to `docs/reports/plans/` during implementation per Constitutional Rule #0.
...
| 5 | Import path verifier | `_verify.py` §5 (structure_blueprint scope only) | Not general-purpose. |
...
10. Move 7 root `.py` files → `config/core/`
11. Rename `env_loader.py` → `env_loader_config.py`
12. Update all importers (≤10 files)
13. Legitimize `agent_configs` with `"type": "spec_data"`, `"no_python": True`
14. Rename `domain_constitution.py` → `domain_constitution_config.py`
15. Fix 5 broken import paths

### Phase 3: Mixins cleanup (Finding 4)

16. Move `structural_healing_engine.py`, `meta_learning_engine.py`, `meta_learning_storage.py` → `agentic_core/utils/`
17. Update 4 importers
18. Rename `inspection_capability.py` → `inspection_capability_mixin.py`, update 11 importers
19. Tighten regex: remove `engine`/`storage` from allowed suffixes

### Phase 4: Prompt governance cleanup (Finding 5)

20. Move 10+ adversarial `.jinja` files → `security/adversarial/`
