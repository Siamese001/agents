...
source_key: windsurf
original_path: 'C:\\Git\\windsurf-plans-recovered\\windsurf_plans\\scorer-otel-autosource-layer-b-c5e4d1.md'
original_relative_path: 'scorer-otel-autosource-layer-b-c5e4d1.md'
source_sha256: e53e6c380f9e00d619f4dae698daa2e145512170f70c675b53d5f2f76660e6b2
...
| B4.1 | Hook wiring + bypass | `.windsurf/scripts/post_cascade_deferred_scope_capture.py` (edit), `tests/unit/windsurf/scripts/test_deferred_scope_capture_autosource.py` (new) | Hook runs on every response — enrichment must be bounded (<500ms total) or timeout; ALL enrichment failures fail-open to v1 scoring | 4000 | Todo |
| B5.1 | Calibration A/B report | `tools/reports/priority_calibration_report.py` (new), `docs/reports/priority/calibration_<date>.md` (artifact) | Need a stable corpus — uses `artifacts/windsurf/deferred_scope_capture.jsonl` tail; threshold recalibration is a follow-up ADR, not a code change here | 3000 | Todo |
