...
source_key: windsurf
original_path: 'C:\\Git\\windsurf-plans-recovered\\windsurf_plans\\apps-rg-l1-contract-wiring-3e7f92.md'
original_relative_path: 'apps-rg-l1-contract-wiring-3e7f92.md'
source_sha256: f6139edf7f46b3e1e3699fbd3f2b69530d58be567f1d1b6b8941be21d3ee1f7b
...
|------|-----------|-------|-------------|--------|
| W0 | P0.1–P0.3 | Baseline verification + import-impact scan + mode map inventory | ~3k | Not Started |
| W1 | P1.1–P1.3 | Core boundary cleanup + contract field safety (5 fields, not 7) | ~4k | Not Started |
| W2 | P2.1–P2.2 | U0 profile manifest wiring (fail-closed) + L1 profile verification | ~4k | Not Started |
| W3 | P3.1 | L1 work-shape hint population (profile-backed constants, advisory route_hints) | ~2k | Not Started |
| W4 | P4.1–P4.5 | CI / governance tests (import scan, boundary, profile, work-shape, non-authority) | ~7k | Not Started |
| W5 | P5.1 | Final receipt: diffs, gate outputs, no-go checklist, determinism statement | ~2k | Not Started |
...
|----------|-------|---------------|-------------|-------------|--------|
| P0.1 | Import-impact scan for L1Planner | Read-only: repo-wide rg scan | Must confirm tombstone is safe before W1 | ~1k | Not Started |
| P0.2 | Baseline contract + synthesizer state | Read-only: 4 files | Confirms live state pre-edit | ~1k | Not Started |
| P0.3 | Generation mode inventory | Read-only: payload_synthesizer.py + l1_binding.py | List all modes emitted by U0 for work-shape test coverage | ~1k | Not Started |
| P1.1 | Tombstone or shim L1Planner | `agentic_core/L1_cognition/l1_plan_contract.py` | Import-safe removal; depends on P0.1 finding | ~1k | Not Started |
| P1.2 | Extend `L1PlanContract` — 5 fields only | `agentic_core/runtime/contracts/l1_plan_contract.py` | Backward-compatible; no apps_rg defaults; ref-only validators | ~2k | Not Started |
| P1.3 | Verify `schema_version` / no `plan_version` | Same file | Confirm `plan_version` was never a real field | ~0.5k | Not Started |
| P2.1 | Add profile ref + fail-closed digest to synthesizer | `apps_rg/runtime/u0/payload_synthesizer.py` | Must raise on missing file in runtime; test_mode flag for tests | ~2k | Not Started |
| P2.2 | L1 binding: verify U0-declared profile digest (fail-closed) | `apps_rg/runtime/bindings/l1_binding.py` | Reject empty digest unless explicitly test-scoped | ~2k | Not Started |
| P3.1 | Work-shape hints + advisory route_hints (vocabulary-safe) | `apps_rg/runtime/bindings/l1_binding.py` | Mode constants in apps_rg only; unknown mode → conservative False | ~1.5k | Not Started |
| P4.1 | Core boundary test (LEGACY_SHIM marker required) | `tests/governance/test_apps_rg_l1_core_boundary.py` | Scanner checks marker presence, not just file name | ~2k | Not Started |
| P4.2 | U0→L1 profile ref + fail-closed tests | `tests/_apps_contract/test_apps_rg_l1_profile_wiring.py` | 5 tests including missing-profile and mismatch rejection | ~2k | Not Started |
| P4.3 | Work-shape hint test (all known modes) | `tests/_apps_contract/test_apps_rg_l1_work_shape.py` | Every mode emitted by U0 has a test case | ~1.5k | Not Started |
| P4.4 | Non-authority / ref-only tests | `tests/_apps_contract/test_apps_rg_l1_non_authority.py` | AST import scan + ref format validators | ~2k | Not Started |
| P4.5 | Route-authority guardrail test | Inline in P4.4 or separate | Assert L1PlanContract has no route selection fields | ~0.5k | Not Started |
| P5.1 | Final receipt / proof bundle | Wave-level summary | Exact diffs, gate outputs, no-go checklist, determinism statement | ~2k | Not Started |
...
| File | Key Finding |
|------|-------------|
| `agentic_core/L1_cognition/l1_plan_contract.py` (lines 1–99) | Dead `L1Planner` class; `apps_rg` literals in method comments; `plan_version="W6.0"` passed to constructor but field is `schema_version` → `TypeError` at call time. **Consumer scan required before tombstoning (W0 P0.1).** |
| `agentic_core/runtime/contracts/l1_plan_contract.py` (lines 1–91) | Canonical frozen dataclass. Missing 5 fields (not 7 — see §2 contract gap). `schema_version` present; `plan_version` absent. Work-shape hint fields exist but never populated by apps_rg. |
| `apps_rg/runtime/bindings/l1_binding.py` (lines 1–330) | App-owned binding. Deterministic. Two gaps: (a) silently discovers profile by filesystem constant; (b) work-shape hints always False. |
| `apps_rg/runtime/u0/payload_synthesizer.py` (lines 205–339) | `profile_manifest` dict has no `l1_planning_profile_ref` or `l1_planning_profile_digest`. `_planning_profile_digest()` does not yet exist. |
| `apps_rg/profiles/rg_planning_profile.yaml` (lines 1–42) | Advisory-only. Correct. No runtime config. |
| `agentic_core/L1_cognition/apps_rg_l1_binding.py` (lines 1–22) | Marked `LEGACY_SHIM`. Re-exports from `apps_rg.runtime.bindings.l1_binding`. Valid under shim policy. |

### 1.2 Field Count Correction (Hardening item 5)
...
|--------|-----------|--------------|
| DoD-1 | `agentic_core/L1_cognition/l1_plan_contract.py` has no unshielded `apps_rg` literals | `grep "apps_rg" agentic_core/L1_cognition/l1_plan_contract.py \| grep -v LEGACY_SHIM` → 0 |
| DoD-2 | `L1PlanContract` has all 5 new fields, 4 validators, no `plan_version` | W1 P1.3 inline check passes |
| DoD-3 | U0 emits 64-char digest; missing profile raises `ProfileManifestError` in runtime | W2 gate passes + `test_u0_missing_profile_raises_in_runtime` |
| DoD-4 | L1 rejects empty digest without env flag; rejects mismatched digest | `test_l1_binding_rejects_empty_digest_without_env_flag` + `test_l1_binding_verifies_profile_digest_mismatch_raises` |
| DoD-5 | Work-shape hints correct for all 5 known modes + unknown | `pytest tests/_apps_contract/test_apps_rg_l1_work_shape.py` → 9 pass |
| DoD-6 | ~30 new CI tests all pass, 0 regressions | `pytest tests/_apps_contract/ tests/governance/ -q` |
| DoD-7 | All 10 no-go checks pass | W5 receipt Section D all PASS |
| DoD-8 | W5 receipt complete with determinism + boundary statements | Both literal sentences present in receipt |
