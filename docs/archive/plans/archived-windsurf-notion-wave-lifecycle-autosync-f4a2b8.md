...
original_relative_path: 'notion-wave-lifecycle-autosync-f4a2b8.md'
source_sha256: 08ac45acf7cb4f0eb6ad13090a35ffe721f1fa8cdd93a458d7ba830aec06a358
...
| `tools/windsurf/wave_execution_state.py` | EXTEND — add `wave-progress --plan <slug> --wave N` subcommand; `start` patches to `In Progress`; `complete` patches to `Completed`. |
| `.windsurf/scripts/post_cascade_wave_lifecycle_capture.py` | NEW — parses `WAVE_START:` / `WAVE_COMPLETE:` / `PHASE_COMPLETE:` / `PLAN_COMPLETE:` markers and shells out to the writer. |
| `.windsurf/hooks.json` | EXTEND — register the new post-cascade hook (one row, schema-pure per §27). |
...
| `tests/unit/windsurf_scripts/test_post_cascade_wave_lifecycle_capture.py` | NEW — marker parsing, shell-out args. |
| `.windsurf/rules/notion-plan-wave-deferral.md` | UPDATE — add §"Sanctioned non-MCP path" section. |
...
| P3.1 | Marker capture hook | `post_cascade_wave_lifecycle_capture.py`: parse 4 marker kinds, shell out per marker | Idempotency — same marker twice in one response = single Notion call | ~3k | Not Started |
| P3.2 | hooks.json wiring | One new `post_cascade_response` row, `show_output: false` | Schema purity §27 — only `command` / `working_directory` / `show_output` | ~2k | Not Started |
...
- **Log**: `artifacts/windsurf/wave_lifecycle_notion.jsonl` — every patch attempt with timestamp, plan, marker, HTTP status.
...
| DoD-2 | Smoke-run | `python tools/windsurf/wave_execution_state.py start --plan <test-slug>` exits 0 AND patches Plans DB to `In Progress` | No |
...
- §31 (SSOT folder routing) — followed; `check_*` → `ops_scripts/ci/`, `post_*` → `.windsurf/scripts/`, writer → `tools/notion/`, helper → `tools/notion/`.
