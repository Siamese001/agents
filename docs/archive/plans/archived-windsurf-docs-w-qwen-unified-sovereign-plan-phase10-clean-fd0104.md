...
original_path: 'C:\\Git\\windsurf-plans-recovered\\docs_reports_plans\\w-qwen-unified-sovereign-plan-phase10-clean-fd0104.md'
original_relative_path: 'w-qwen-unified-sovereign-plan-phase10-clean-fd0104.md'
source_sha256: 25e64fe05ae948dda939f8757a38cfcf5d6eb7390605db950a03bd9beccd89df
...
---
# W-LOCAL-QWEN UNIFIED SOVEREIGN PLAN (PHASE 10 CLEAN)
...
def handle_qwen_oom(
    ...
    """Handle OOM with escalation through router choke point."""
    # Emit FailureSignal for L2.3 consumption
    failure_signal = FailureSignal(
        source_agent=healing_input.agent_id,
        failure_type="gpu_oom",
        error_signature="qwen_gpu_oom",
        trace_id=healing_input.trace_id,
        context={"retry_count": retry_count, "error": "GPU out of memory"},
        retry_count=retry_count,
        blast_radius_estimate=0.1
    )

    # Increment retry count and route through choke point
    ...
    # Force escalation through router (single choke point)
...
def invoke_qwen_with_oom_protection(
    ...
    try:
        ...
        if "out of memory" in str(exc).lower():
            # retry_count >= 3 forces GEMINI tier via router
            if healing_input.retry_count >= 3:
                logger.error("Qwen OOM retry exhausted, forcing GEMINI escalation")
                escalated_decision = handle_qwen_oom(healing_input, healing_input.retry_count, config)
                # Router will return GEMINI_2_5_PRO tier
                raise OOMEscalatedError(f"OOM escalated to {escalated_decision.tier}")
            else:
                # Increment retry and re-route
                new_retry_count = healing_input.retry_count + 1
                escalated_input = HealingInput(
                    agent_id=healing_input.agent_id,
                    failure_type=healing_input.failure_type,
                    error_signature=healing_input.error_signature,
                    trace_id=healing_input.trace_id,
                    retry_count=new_retry_count,
                    blast_radius_estimate=healing_input.blast_radius_estimate,
                    required_tools=healing_input.required_tools,
                    violation_metadata_refs=healing_input.violation_metadata_refs
                )
                escalated_decision = route_healing_tier(escalated_input, config)
                raise OOMRetryableError(f"OOM retry {new_retry_count}/3, escalated to {escalated_decision.tier}")
        raise
```
...
def validate_qwen_startup_state() -> None:
    ...
    if not qwen_enabled:
        # Assert no Qwen processes are running
...
def is_vllm_process_running() -> bool:
    """Check if any vLLM processes are currently running."""
    try:
...
This unified plan replaces all previous Qwen hardening drafts and provides the authoritative Phase 10 sovereign-compliant specification for HealingTier.QWEN_VLLM integration.
