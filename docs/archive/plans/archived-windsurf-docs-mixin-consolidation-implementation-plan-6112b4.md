...
original_path: 'C:\\Git\\windsurf-plans-recovered\\docs_reports_plans\\mixin-consolidation-implementation-plan-6112b4.md'
original_relative_path: 'mixin-consolidation-implementation-plan-6112b4.md'
source_sha256: 396df7d00cea6e5ab92f8ad09cf7cc2e3a7bfd33af06eb09ef72e9e7462f9dbd
...
> **SSOT Note**: Final copy of this plan MUST be saved to `docs/reports/plans/` per Constitutional Rule #0.
...
| `agentic_core/mixins/caching_mixin.py` | Replace with §26 shim | 2 |
| `agentic_core/mixins/batching_mixin.py` | Replace with §26 shim | 3 |
| `agentic_core/mixins/metrics_mixin.py` | Replace with §26 shim | 4 |
| `agentic_core/base_agents/LightweightBase.py` | Fix missing imports, replace CachingMixin+MetricsMixin with PerformanceMixin | 5 |
| `agentic_core/base_agents/SovereignBaseAgent.py` | Remove duplicate SubatomicTestingMixin from bases | 6 |
| `agentic_core/runtime/utils/trait_system_util.py` | Update CachingTrait/MetricsTrait imports to use PerformanceMixin types | 7 |
| `tests/_quarantine/integration/core_dir/test_mro_refactoring_integration.py` | Update test assertions for shim behavior | 8 |
| `tests/e2e/misc/test_mro_refactoring_e2e.py` | Update test assertions for shim behavior | 8 |
| `tests/agentic_core/mixins/test_mixin_consolidation_regression.py` | **NEW** — regression test suite | 9 |

**Total files modified**: 9 existing + 1 new test file
...
### Sub-phase 1.4: Update `__all__` export
...
Wait — we should NOT re-export the deprecated names from `performance_mixin.py`. The shim files will handle backward-compat naming. `performance_mixin.py` exports only its own canonical names.
...
## Phase 2: Convert `CachingMixin` to §26 Shim

### Sub-phase 2.1: Replace file content
...
from agentic_core.mixins.performance_mixin import (
    CacheEntry,
    PerformanceConfig as CacheConfig,
...
**Key decisions**:
- `CacheConfig` is aliased from `PerformanceConfig` — consumers that imported `CacheConfig` get a working config object (superset of fields).
- `CacheEntry` is re-exported from `performance_mixin.py` where the canonical copy lives.
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P2-T1 | `from agentic_core.mixins.caching_mixin import CachingMixin` succeeds | `pytest tests/agentic_core/mixins/test_caching_mixin.py -xvs` |
| P2-T2 | `CachingMixin is PerformanceMixin` | regression test |
| P2-T3 | `isinstance(PerformanceMixin(), CachingMixin)` is True | regression test |
| P2-T4 | `CacheEntry` import from shim resolves | regression test |
| P2-T5 | §26 AST lock: no ClassDef or FunctionDef in shim | regression test |
| P2-T6 | Existing `test_caching_mixin.py` passes | `pytest tests/agentic_core/mixins/test_caching_mixin.py -xvs` |
...
## Phase 3: Convert `BatchingMixin` to §26 Shim
...
from agentic_core.mixins.performance_mixin import (
    PerformanceConfig as BatchingConfig,
    PerformanceMixin as BatchingMixin,
)
...
**Key decisions**:
- `BatchingConfig` aliased from `PerformanceConfig` (superset).
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P3-T1 | `from agentic_core.mixins.batching_mixin import BatchingMixin` succeeds | `pytest tests/agentic_core/mixins/test_batching_mixin.py -xvs` |
| P3-T2 | `BatchingMixin is PerformanceMixin` | regression test |
| P3-T3 | §26 AST lock: no ClassDef or FunctionDef in shim | regression test |
| P3-T4 | Existing `test_batching_mixin.py` passes | `pytest tests/agentic_core/mixins/test_batching_mixin.py -xvs` |
...
## Phase 4: Convert `MetricsMixin` to §26 Shim
...
from agentic_core.mixins.performance_mixin import (
    PerformanceConfig as MetricsConfig,
    PerformanceMetrics,
...
**Key decisions**:
- `PerformanceMetrics` is re-exported (canonical copy in `performance_mixin.py`). Eliminates EXACT-duplicate dataclass.
- `MetricsConfig` aliased from `PerformanceConfig` (superset).
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P4-T1 | `from agentic_core.mixins.metrics_mixin import MetricsMixin` succeeds | `pytest tests/agentic_core/mixins/test_metrics_mixin.py -xvs` |
| P4-T2 | `MetricsMixin is PerformanceMixin` | regression test |
| P4-T3 | `from agentic_core.mixins.metrics_mixin import PerformanceMetrics` succeeds | regression test |
| P4-T4 | §26 AST lock: no ClassDef or FunctionDef in shim | regression test |
| P4-T5 | Existing `test_metrics_mixin.py` passes | `pytest tests/agentic_core/mixins/test_metrics_mixin.py -xvs` |
...
## Phase 5: Update `LightweightAgentBase` MRO

### Pre-existing Bug

`LightweightBase.py` lines 44-49 reference `CostGuardrailMixin`, `ContextManagementMixin`, `TracingMixin`, `CachingMixin`, `MetricsMixin` as class bases but **never imports them**. The `__post_init__` manually initializes state. This works only because the file is never actually imported at runtime in production (or the imports are injected elsewhere). This phase fixes the bug AND updates the MRO.

### Sub-phase 5.1: Full file diff
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P5-T1 | `from agentic_core.base_agents.LightweightBase import LightweightAgentBase` succeeds | `python -c "from agentic_core.base_agents.LightweightBase import LightweightAgentBase"` |
| P5-T2 | `LightweightAgentBase` subclass has `cache_get`, `cache_set` | regression test |
| P5-T3 | `LightweightAgentBase` subclass has `record_timing`, `get_metrics` | regression test |
| P5-T4 | `LightweightAgentBase` subclass has `batch_add`, `batch_flush` | regression test |
| P5-T5 | MRO depth < 10 (was ~8, now ~6) | regression test |
| P5-T6 | `verify_lightweight_state()` returns True | regression test |
| P5-T7 | E2E test `test_lightweight_base_is_functional` passes | `pytest tests/e2e/misc/test_mro_refactoring_e2e.py::TestPhase4E2E -xvs` |
...
## Phase 6: Remove Duplicate `SubatomicTestingMixin` from `SovereignBaseAgent`

### Sub-phase 6.1: Remove duplicate base
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P6-T1 | `SovereignBaseAgent.__mro__` contains `SubatomicTestingMixin` exactly once | regression test |
| P6-T2 | `from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent` succeeds | `python -c "from agentic_core.base_agents.SovereignBaseAgent import SovereignBaseAgent"` |
| P6-T3 | `hasattr(SovereignBaseAgent, 'run_subatomic_tests')` still True (inherited from infrastructure_mixin) | regression test |
...
## Phase 7: Update `trait_system_util.py` Imports

### Sub-phase 7.1: Update CachingTrait and MetricsTrait imports

`CachingTrait` imports `CacheConfig`, `CacheEntry` from `caching_mixin.py`. `MetricsTrait` imports `MetricsConfig`, `PerformanceMetrics` from `metrics_mixin.py`. These shims will still work (they re-export), but for clarity update to canonical imports.
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P7-T1 | `from agentic_core.runtime.utils.trait_system_util import CachingTrait` succeeds | import check |
| P7-T2 | `CachingTrait.apply(TestClass)` produces class with `cache_get` | regression test |
| P7-T3 | `MetricsTrait.apply(TestClass)` produces class with `record_timing` | regression test |

---

## Phase 8: Update Integration and E2E Tests

### Sub-phase 8.1: Quarantined integration test
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P8-T1 | All quarantined integration tests pass | `pytest tests/_quarantine/integration/core_dir/test_mro_refactoring_integration.py -xvs` |
| P8-T2 | All E2E tests pass | `pytest tests/e2e/misc/test_mro_refactoring_e2e.py -xvs` |

---

## Phase 9: Create Consolidation Regression Test Suite

### Sub-phase 9.1: New test file
...
4. LightweightAgentBase MRO is correct
5. SovereignBaseAgent MRO has no duplicate bases
"""
...
| Test ID | Description | Command |
|---------|-------------|---------|
| P9-T1 | Full regression suite passes | `pytest tests/agentic_core/mixins/test_mixin_consolidation_regression.py -xvs` |
| P9-T2 | All 4 §26 checks × 3 shims = 12 pass | subset of above |
| P9-T3 | All 3 identity checks pass | subset of above |
| P9-T4 | All union API parametrized checks pass | subset of above |
| P9-T5 | All functional tests pass | subset of above |
...
Phase 1 → Phase 2 → Phase 3 → Phase 4 → Phase 5 → Phase 6 → Phase 7 → Phase 8 → Phase 9
  │                                                                                │
  │  GATE: pytest tests/agentic_core/mixins/test_performance_mixin.py -xvs        │
...
Each phase MUST pass its gate tests before the next phase begins. If any gate fails → STOP, root-cause, fix, re-run same phase.
...
| Risk | Mitigation |
|------|------------|
| `CacheConfig` has different fields than `PerformanceConfig` (alias) | `PerformanceConfig` is a superset — `CacheConfig.enabled` maps to `PerformanceConfig.cache_enabled`. The alias works for construction but field names differ. Consumers accessing `.enabled` will get `AttributeError`. **Mitigated**: Only `LightweightBase.__post_init__` and `CachingMixin.__init__` construct `CacheConfig` directly — both are replaced. |
| `BatchingConfig` has different field names | Same as above — replaced in Phase 3. Only `BatchingMixin.__init__` constructs it. |
| Thread-safety regression | `PerformanceMixin` uses `_perf_lock` for all operations — same pattern as split mixins. No regression. |
| `isinstance` checks in decorators | `CachingMixin.cached` checks `isinstance(self, CachingMixin)`. Post-shim, `CachingMixin IS PerformanceMixin`, so any `PerformanceMixin` instance passes. This is **correct** — wider acceptance, no false negatives. |
| E2E test checks file paths that don't exist | Fixed in Phase 8 — paths updated to actual locations. |
