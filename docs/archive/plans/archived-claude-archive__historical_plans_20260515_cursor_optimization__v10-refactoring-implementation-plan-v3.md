...
source_key: claude
original_path: 'C:\\Git\\Agentic-Workflow-FRESH\\.codex\\plans\\_archive\\historical_plans_20260515_cursor_optimization\\v10-refactoring-implementation-plan-v3.md'
original_relative_path: '_archive\\historical_plans_20260515_cursor_optimization\\v10-refactoring-implementation-plan-v3.md'
source_sha256: 33c8186f220c67ce05f51f1175ffa4d852ea6a71d88cd7268269556489b6c2a3
...
---
# V10 Architectural Transformation Implementation Plan v3.1

A fully deterministic, zero-flake approach to the Zero-Loss refactoring of 171 agents with automated MRO enforcement, deterministic behavioral harnesses, granular blast-radius control, and immediate concurrency verification.
...
### CONCURRENCY VERIFICATION RULE (MANDATORY)

> **v3.1 NEW:** Any agent receiving `AtomicExecutionMixin` MUST pass concurrency verification BEFORE proceeding to next wave.

```bash
# Required BEFORE completing any wave with AtomicExecutionMixin
...
#### 2.1.5 Concurrency Verification (REQUIRED BEFORE WAVE 3)

> **v3.1 NEW:** Since `DomainPlannerAgent` now has `AtomicExecutionMixin`, we MUST verify file locking works under concurrent access BEFORE proceeding to Wave 3.

- [ ] Create `tests/stress/test_atomic_concurrency.py` with DomainPlanner-specific test
...
```bash
python -m pytest tests/stress/test_atomic_concurrency.py::TestDomainPlannerConcurrency -v --tb=long
```

- **Pass Criteria**: Zero data corruption, all threads complete or rollback cleanly
- **If FAIL**: STOP Wave 2 and investigate `AtomicExecutionMixin` implementation

#### 2.1.6 Behavioral Verification (Golden Output Test)
...
#### 2.1.7 Deprecate DomainPlannerAdapter
...
- [ ] Run existing concurrency test: `tests/stress/test_atomic_concurrency.py`
- [ ] Verify **all** agents with `AtomicExecutionMixin` pass
- [ ] Spawn **5 threads** targeting the same file simultaneously
- [ ] Verify file locking prevents race conditions
- [ ] Verify rollback works when one thread fails
- **Test Command**:
...
- [ ] Run concurrency test for any agents receiving `AtomicExecutionMixin`
...
7. [ ] Run concurrency test if `AtomicExecutionMixin` added
...
- [ ] Run concurrency test for any new `AtomicExecutionMixin` agents
- **Command**: `pytest tests/guardian/ -v --tb=short`
- **Command**: `pytest tests/behavioral/verify_golden.py -v`
- **Command**: `pytest tests/stress/test_atomic_concurrency.py -k "new_agents"`
...
- [ ] Run concurrency test after adding `AtomicExecutionMixin`
...
- [ ] Run concurrency test for any `AtomicExecutionMixin` agents
...
### Phase 8.4: Final Concurrency Verification

- [ ] Run full concurrency test suite for ALL agents with `AtomicExecutionMixin`
- [ ] Verify no race conditions across entire codebase
- [ ] **Command**: `pytest tests/stress/test_atomic_concurrency.py -v --tb=long`
...
# 3. If AtomicExecutionMixin added, run concurrency test
pytest tests/stress/test_atomic_concurrency.py -k "<AgentName>"
...
- Guardian tests: PASS
- Unit tests: X/Y PASS
- Concurrency test: PASS (if applicable)"
```
...
## CONCURRENCY VERIFICATION (v3.1 NEW)

> **CRITICAL:** Any agent receiving `AtomicExecutionMixin` MUST pass concurrency verification BEFORE proceeding to next wave.

### Test Implementation

**File**: `tests/stress/test_atomic_concurrency.py`
...
class TestAtomicConcurrency:
    ...
    def test_concurrent_file_access(
        ...
        """
        # Use a mock agent with AtomicExecutionMixin for testing
        class MockAtomicAgent:
            def __init__(self):
    ...
    def _verify_concurrent_results(self, results: List[Dict], test_file: Path):
        ...
        if failed:
            rollback_occurred = any(r["rollback_occurred"] for r in failed)
...
| Wave | When to Run | Scope |
|------|-------------|-------|
| Wave 2 | **After DomainPlannerAgent gets AtomicExecutionMixin** | DomainPlannerAgent ONLY |
| Wave 3 | After each batch with AtomicExecutionMixin | All agents in batch |
| Wave 4 | After adding AtomicExecutionMixin to WorkflowEngine | WorkflowEngine only |
| Wave 6 | After each batch with AtomicExecutionMixin | All agents in batch |
| Wave 8 | Final verification | ALL agents with AtomicExecutionMixin |

### Pass/Fail Criteria

**PASS:**
- Zero data corruption
- All threads complete or rollback cleanly
...
- Rollback not triggered on failure
...
├── stress/
│   └── test_atomic_concurrency.py  # NEW: Concurrency tests
├── guardian/
...
- [ ] **Concurrency tests pass for all AtomicExecutionMixin agents**
...
| Mixin | Purpose | Apply To | Concurrency Test Required |
|-------|---------|----------|---------------------------|
| `AtomicExecutionMixin` | Rollback on failure | Healers, Validators | **YES** |
| `AuditTrailMixin` | Audit logging | All agents | NO |
| `CircuitBreakerMixin` | Timeout protection | External calls | NO |
| `HallucinationDetectionMixin` | Verify targets exist | Healers | NO |
| `MCPHardenedMixin` | MCP protocol safety | L2 agents | NO |
| `RateLimitMixin` | API rate limiting | External touch | NO |
| `RedisCacheMixin` | Redis caching | Memory agents | NO |
| `SubatomicTestingMixin` | Built-in tests | All agents | NO |
| `MetaLearningMixin` | Learn from outcomes | L1 agents | NO |
| `TracingMixin` | Distributed tracing | All agents | NO |
...
### v3.1 (2026-02-03) - IMMEDIATE CONCURRENCY VERIFICATION

**Based on Final Risk Assessment**

#### Critical Adjustment

1. **Concurrency Verification Moved Up**: Since `DomainPlannerAgent` receives `AtomicExecutionMixin` in Wave 2, concurrency test `tests/stress/test_atomic_concurrency.py` must be created and passed **NOW**, not in Wave 3.

#### Changes to Wave 2

- **NEW Step 2.1.5**: Concurrency Verification (required BEFORE Wave 3)
- Added DomainPlanner-specific concurrency test
- **IF FAIL**: STOP Wave 2 and investigate `AtomicExecutionMixin` implementation

#### Changes to Subsequent Waves

- Wave 3: Run concurrency test for each batch receiving `AtomicExecutionMixin`
- Wave 4: Run concurrency test after adding to WorkflowEngine
- Wave 6: Run concurrency test for any batch with `AtomicExecutionMixin`
- Wave 8: Added Phase 8.4 for final concurrency verification

#### New Section Added

- **CONCURRENCY VERIFICATION** - Complete test implementation and requirements
- Updated Mixin Reference table with "Concurrency Test Required" column
